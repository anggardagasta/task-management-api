...
describe('Task Service', () => {
    ...
    describe('getTaskStatusSummary', () => {
        it('should return task status summary without search', async () => {
            const mockTasks = [
                {status: 'To Do'},
                {status: 'In Progress'},
                {status: 'To Do'},
            ...
            (getCosmosContainer as jest.Mock).mockResolvedValue(mockContainer);
            mockQuery.mockReturnValueOnce({
                // fetchAll: jest.fn().mockResolvedValue({ resources: mockStatuses.map(s => ({ status: s })) }),
                fetchAll: jest.fn().mockResolvedValue({resources: ['To do', 'In Progress', 'Completed', 'Cancelled']}),
            });
            mockQuery.mockReturnValueOnce({
                fetchAll: jest.fn().mockResolvedValue({resources: mockTasks}),
            });

            const {statuses, tasks} = await getTaskStatusSummary('org1', {});

            expect(tasks).toEqual(mockTasks);
            expect(statuses).toEqual(['to_do', 'in_progress', 'completed', 'cancelled']);
        });

        it('should return task status summary with search criteria', async () => {
            const mockTasks = [
                {status: 'To Do'},
                {status: 'In Progress'},
                {status: 'To Do'},
            ...
            (getCosmosContainer as jest.Mock).mockResolvedValue(mockContainer);
            mockQuery.mockReturnValueOnce({
                fetchAll: jest.fn().mockResolvedValue({resources: ['To Do', 'In Progress']}),
            });
            mockQuery.mockReturnValueOnce({
                fetchAll: jest.fn().mockResolvedValue({resources: mockTasks}),
            });

            const search = {title: 'Task 1'};
            const {statuses, tasks} = await getTaskStatusSummary('org1', search);

            expect(tasks).toEqual(mockTasks);
            expect(statuses).toEqual(['to_do', 'in_progress']);
            expect(mockQuery.mock.calls[0][0].query).toContain('SELECT DISTINCT VALUE LOWER(c.status) FROM c WHERE c.organizationId = @organizationId AND IS_DEFINED(c.status)');
            expect(mockQuery.mock.calls[0][0].parameters).toEqual([{name: '@organizationId', value: 'org1'}]);

            expect(mockQuery.mock.calls[1][0].query).toContain('CONTAINS(LOWER(c.title), LOWER(@title))');
            expect(mockQuery.mock.calls[1][0].parameters).toEqual([{
                name: '@organizationId',
                value: 'org1'
            }, {name: '@title', value: 'Task 1'}]);
        });
    });
});